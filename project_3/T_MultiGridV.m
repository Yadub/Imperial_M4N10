function Tout = T_MultiGridV(Tin, f, dt, dr, dtheta, r)
...
if ( (N==2) || (M==2))
    Tout = T_GaussSeidel( Tin, f, dt, dr, dtheta, r, iters ); 
else
    % Otherwise begin the cycle from fine to coarsest

    % Start by smoothing input with 10 GS iterations
    Tsmooth = T_GaussSeidel( Tin, f, dt, dr, dtheta, r, iters ); 

    % Compute the residuals on a coarser grid
    res  = T_residual(Tsmooth, f, dt, dr, dtheta, r); 
    ...
    err = T_MultiGridV( zeros(size(res2)), res2 , dt, dr2, dtheta2, r2); 
    ...
    Tout = T_GaussSeidel( Tsmooth, f, dt, dr, dtheta, r, iters ); 
end
