...
\begin{enumerate}
   ...
   32& 0.0825  &  0.1543 &   0.3196  &  0.8266\\
	...
	\item Given that all 3 algorithm are equally accurate based on the previous tests, I have used a MultiGrid routine to compute the solution for a grid of size $256 \times 256s$ and $b=5$. Complications might arise when dealing with the second problem (b) because the Dirichlet boundary condition is not $2\pi $ periodic as it is of the form $\sin(p \theta)$ but now $p$ is not an integer. 
